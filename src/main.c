...
int main(void)
{
	...
	glLoadIdentity();
	glFrustum(-NEAR_PLANE * WIN_ASPECT,
		  NEAR_PLANE * WIN_ASPECT,
		  -NEAR_PLANE,
		  NEAR_PLANE,
		  NEAR_PLANE,
		  FAR_PLANE);
	...
	while (1) {
		...
		assertf(current_room < 100,
				"Congratulations, you got through 100
		rooms.\n");
				*/
		...
		while (ticks_accum >= DELTATICKS) {
			static joypad_inputs_t held_old = { 0 };
			joypad_inputs_t	       held, down = { 0 };

			joypad_poll();
			held = joypad_get_inputs(JOYPAD_PORT_1);
#define BTN_DIFF(_btn) down.btn._btn = held.btn._btn ^ held_old.btn._btn
			BTN_DIFF(a);
			BTN_DIFF(b);
			BTN_DIFF(c_left);
			BTN_DIFF(c_right);
...
#undef BTN_DIFF
			...
			const int16_t stick[2] = { held.stick_x, held.stick_y };

			static bool trigd	 = false;
			bool	    room_changed = current_room != last_room;
			bool	    cur_room_has_lockers =
					lockers_cnt[rooms[current_room].index] >
					0;
			bool prev_room_has_lockers =
					lockers_cnt[rooms[current_room - 1]
								    .index] > 0;
			if (room_changed && !trigd &&
			    (cur_room_has_lockers || prev_room_has_lockers)) {
				int ran = rand() % 5;
				if (ran == 0) {
					// debugf("Triggered\n");
